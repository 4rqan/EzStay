...
route.post(
  ...
  asyncHandler(async (req, res) => {
    const { worker, noOfDays, startDate, workType, comment, location } =
      req.body;
    ...
    const replacements = {
      ...
      "[MESSAGE2]": `${
        req.user.fullname
      } has booked your service from ${new Date(
        startDate
...
route.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (newDate < booking.startDate) {
      return res
        .status(400)
        .send("Date should not be less than the start date");
    }
...
route.put(
  "/workerbooking/attendance/reject",
  requireAuth,
  asyncHandler(async (req, res) => {
